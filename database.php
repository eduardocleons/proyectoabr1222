...
class Database{
    ...
    public function __construct($dbhost, $dbuser, $db_pass, $db_name){
        ...
        if($this->db->connect_errno){
            echo "error<br>Fallo la conexion con Mysql, tipo de error -> ({$this->db->connect_error}) <a href='index.html'>Regresar</a>";  
        }
    }
    ...
    public function preparar($consulta){
        ...
        if(!$this->prep){
            echo("Error al preparar la consulta. <a href='index.html'>Regresar</a>");
        } 
    }
    ...
    public function read(){
        $this->resultado= $this->db->query("SELECT id,nombre, email, sexo, area_id, boletin, descripcion FROM empleados");
        $ch= $this->resultado->fetch_all(MYSQLI_NUM);
        //print_r($ch);
        return $ch; 
    }

    public function data_modify($v_id){
        $this->resultado= $this->db->query("SELECT id,nombre, email, sexo, area_id, boletin, descripcion FROM empleados where id=".$v_id);
        $data[0]= $this->resultado->fetch_all(MYSQLI_NUM);
        $this->resultado= $this->db->query("SELECT rol_id FROM empleado_rol where empleado_id=".$v_id);
        $data[1]= $this->resultado->fetch_all(MYSQLI_NUM);
        return $data; 
    }
